...
export async function POST(request: NextRequest) {
  try {
        ...
        try {
          ...
          const { data: generation } = await supabase
            ...
            .insert({
              user_id: user.id,
              input_type: "raw_text",
              input_content: `Generated via Gemini prompt-engine (${format})`,
          ...
          if (message.includes("SAFETY") || message.includes("safety")) {
            ...
            controller.enqueue(
              encoder.encode(
                `data: ${JSON.stringify({ error: "Gemini API rate limit reached. Please wait a moment and try again." })}\n\n`
              )
            );
          } else {
            controller.enqueue(
    ...
    if (err.message?.includes("API_KEY")) {
      return NextResponse.json(
        { error: "Gemini API key is not configured. Contact support." },
        { status: 500 }
      );
    }
